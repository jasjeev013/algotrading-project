...
const RegimeHeader = () => {
  ...
  useEffect(() => {
    ...
    const fetchRegime = async () => {
      try {
        const res = await axios.get('http://localhost:8000/api/regime', { timeout: 8000 })
        if (!cancelled) {
          setRegime(res.data)
          setError(null)
        }
  ...
  if (!regime) {
    return <div className="regime-header regime-header-loading">Loading market regime…</div>
  }

  const changeClass = regime.spy_daily_change_pct >= 0 ? 'positive' : 'negative'
    ...
    <div className="regime-header">
      <div className="regime-item">
        ...
        <span className={`regime-change ${changeClass}`}>
          {regime.spy_daily_change_pct >= 0 ? '+' : ''}{regime.spy_daily_change_pct.toFixed(2)}%
        </span>
      </div>
      <div className="regime-item">
        <span className="regime-label">VIX</span>
        <span className={`regime-value vix-dot ${vixColorClass(regime.vix_level)}`}>
